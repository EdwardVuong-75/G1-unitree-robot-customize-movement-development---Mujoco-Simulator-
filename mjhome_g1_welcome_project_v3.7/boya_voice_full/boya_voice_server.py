...
def record_boya(seconds=5):
    ...
    cmd_convert = [
        ...
        "-ac", "1",
        "-ar", "16000",
        "-af", "highpass=f=120,lowpass=f=7000,dynaudnorm=f=150:g=5:p=0.70,volume=3dB,alimiter=limit=0.95",
        str(wav_path),
    ]
...
def speak_result(result):
    """
    Speak the actual current reply text.
    It asks the laptop agent to generate a WAV using Windows TTS,
    then converts that WAV to the existing Unitree speaker file greeting_1_16k.wav,
    then calls mjhome_say.py eth0 1 to send audio through the known working speaker path.
    """
    if not result:
    ...
    try:
        ...
        target_wav = Path("/home/unitree/unitree_sdk2_python/greeting_1_16k.wav")
        backup_wav = Path("/home/unitree/unitree_sdk2_python/greeting_1_16k.wav.original_backup")

        payload_path.write_text(json.dumps({"text": reply_text}, ensure_ascii=False), encoding="utf-8")

        url = LAPTOP_AGENT_URL.rstrip("/") + "/api/tts_wav"
        add_log(f"Requesting dynamic TTS from laptop: {url}")
        subprocess.run([
            "curl",
            "-s",
            "-X", "POST",
        ...
        if not laptop_wav.exists() or laptop_wav.stat().st_size < 1000:
            raise RuntimeError("Downloaded TTS wav is missing or too small.")

        # Backup original old greeting once.
        if target_wav.exists() and not backup_wav.exists():
            subprocess.run(["cp", str(target_wav), str(backup_wav)], check=True)
            add_log(f"Original greeting backed up to {backup_wav}")

        # Convert dynamic reply to the exact file used by mjhome_say.py choice 1.
        subprocess.run([
            "ffmpeg",
            "-y",
            "-loglevel", "error",
        ...
        add_log(f"Dynamic reply audio prepared: {target_wav}")
        add_log(f"Speaking dynamic reply: {reply_text}")

        run_async(
            ["python3", MJHOME_SAY_SCRIPT, NETWORK_INTERFACE, "1"],
            label="speak_dynamic_reply",
            timeout=120,
        )

        return True, "Speaking dynamic reply."

    except Exception as e:
        add_log(f"Dynamic TTS failed, fallback to old fixed speech: {e}")
        ...
        run_async(
            ...
            label=f"speak_fallback_{choice}",
            timeout=120,
        )
        return True, f"Dynamic TTS failed; fallback speech choice {choice} started."
