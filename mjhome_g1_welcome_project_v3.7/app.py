...
def draw_pose(frame):
    """If pose detection is enabled, draw the skeleton and trigger arm presets."""
...
def set_motion_enabled(value: bool):
    global motion_enabled
    motion_enabled = bool(value)
    add_log(f"Motion enabled (voice): {motion_enabled}")
...
voice_controller = VoiceController(
    move_fn=move_robot_command,
    arm_fn=arm_command,
    say_fn=say_phrase,
    motion_toggle_fn=set_motion_enabled,
    log_fn=add_log,
)
...
@app.route("/api/voice_enable", methods=["POST"])
def api_voice_enable():
    data = request.get_json(silent=True) or {}
    enabled = bool(data.get("enabled", True))
    if enabled:
        ok, message = voice_controller.start()
    else:
        ok, message = voice_controller.stop()
    return jsonify({"ok": ok, "message": message})
...
def api_voice_status():
    return jsonify({
        "ok": True,
        "running": voice_controller.is_running,
        "last_transcript": voice_controller.last_transcript,
    })
